...
reviewRouter.delete('/reviews/:id', requireAuth, async (request, response, next) => {
  try {
    ...
    if (review.userId.toString() !== request.authenticatedUser!.id && request.authenticatedUser!.role !== 'admin') throw new AppError(403, 'Du får bara ta bort din egen recension.')
