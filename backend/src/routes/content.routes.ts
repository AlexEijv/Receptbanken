...
contentRouter.get('/admin/reports', requireAuth, requireRole('admin'), async (_request, response, next) => { try { const reports = await ReportModel.find({ status: 'open' }).populate('recipeId', 'title').populate('userId', 'username email').sort({ createdAt: -1 }).lean().exec(); response.json({ success: true, data: reports }) } catch (error) { next(error) } })