...
import { createCategory, deleteAdminRecipe, deleteAdminUser, deleteCategory, fetchAdminRecipes, fetchAdminUsers, fetchCategories, type AdminUser, type Category, updateCategory } from '../services/api'
...
<template>
  <main class="admin-page">
    ...
    <template v-else>
      ...
      <section v-if="activeTab === 'users'" class="admin-table-section"><div class="admin-table"><div v-for="user in users" :key="user.email" class="admin-row admin-row--managed"><div><strong>{{ user.username }}</strong><span>{{ user.email }}</span></div><small>{{ user.role }}</small><button v-if="user.role !== 'admin'" class="danger-button" type="button" @click="removeUser(user)">Ta bort</button></div><p v-if="!users.length" class="empty-inline">Inga användare hittades.</p></div></section>
